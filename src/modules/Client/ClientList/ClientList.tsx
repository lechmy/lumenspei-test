    ...
    <section>
      <div className='flex justify-between items-center al my-4'>
        <Button className="ml-2" onClick={handleCreateNew}>
          <UserPlusIcon className="h-5 w-5" aria-hidden="true" title="Add new Client"/>
        </Button>
        <Input
          name="search"
          value={searchQuery}
      ...
      {isLoading && (
        <ul role="list" className="divide-y divide-gray-100 mt-4">
          {clients.map((client) => (
            <li key={client.email} className="flex justify-between flex-col md:flex-row gap-x-6 py-5">
              <div className="flex min-w-0 gap-x-4">
                <div className="min-w-0 flex-auto">
                  <p className="text-sm font-semibold leading-6 text-gray-900">{`${client.firstName} ${client.lastName}`}</p>
              ...
              <div className="flex shrink-0 items-center mt-2 md:mt-0">
                ...
                <Button className="ml-2" onClick={() => handleDelete(client)}>
